...
\begin{tikzpicture}
	...
	\newcommand{\LP}{Can one or more variables $L$ predict participation?}
	\newcommand{\nB}{Bias is unlikely}
	...
	\newcommand{\CCDL}{Do outcome $D$ and $L$
		have an unobserved
		common cause $U$, or does L cause D?}	

	\node (A) {\LP}
	child [L2] {[] node  (B1) {\LED}
				child [L2] {[rounded corners=3mm] node (C1) {IPPW}}
			    child [L2] {[] node (C2) {\CCDL}
		                    child [L2] {[] node (D1) {\EL}
		                    	        child [L2] {[rounded corners=3mm] node (E1) {IPPW}}
		                    	        child [L2] {[] node (E2) {\EP}
		                    	        	        child [L2] {[rounded corners=3mm] node (F1) {IPPW, AR}}
