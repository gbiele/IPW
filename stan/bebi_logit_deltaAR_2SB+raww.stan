...
generated quantities {
  ...
  for (k in 1:K_raw) {
    me_ipw[k] = dot_product(inv_logit(X0[k][,1:K_ipw] * b_ipw + alpha_ipw) - 
                            inv_logit(X1[k][,1:K_ipw] * b_ipw + alpha_ipw),
                            weights[get_iter(id)+25])/N;
    me_raw[k] = dot_product(inv_logit(X0[k][,1:K_raw] * b_raw + alpha_raw) - 
                            inv_logit(X1[k][,1:K_raw] * b_raw + alpha_raw),
                            weights[get_iter(id)+25])/N;
    me_ar[k] = dot_product(inv_logit(X0[k] * b_ar + alpha_ar) - 
