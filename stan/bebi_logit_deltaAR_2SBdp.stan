...
transformed parameters {
  ...
  for (k in 1:K_ipw) {
    ...
    vector[N] m1_ipw = m0_ipw + X[,k] * b_ipw[k] + alpha_ipw;
    vector[N] m0_ar  = X[,didx[k,]] * b_ar[didx[k,]] + alpha_ar;
    vector[N] m1_ar  = m0_ar +  X[,k] * b_ar[k] + alpha_ar;
    
    delta_me_p[k] = mean( inv_logit(m0_ipw) - inv_logit(m1_ipw) - inv_logit(m0_ar) + inv_logit(m1_ar) );
  }
